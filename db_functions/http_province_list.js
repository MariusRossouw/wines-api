create or replace function http_province_list(http_req_text text) returns JSON as
$$
if (!plv8.ufn){
  ...
  var s_count = "select count(p.id) cnt from tb_province p " + where + end;
  ...
  var s_query = " \
    ...
    p.update_date, \
    p.update_time, \
    p.update_display_time \
    from tb_province p " + where + limit + offset + end;
