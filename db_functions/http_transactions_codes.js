create or replace function http_transactions_codes(http_req_text text) returns JSON as
$$
if (!plv8.ufn){
...
// ****************** code ******************
  var sql = `select x.name, sum(x.value) as value
from(
  select distinct mer.code as name,
    (
  ...
  sql += `) as value
    from tb_merchant mer ) x group by x.name `;
